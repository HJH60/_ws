...
export function list(posts, user) {
  ...
  for (let post of posts) {
    list.push(`
    <li>
      <h2>${ post.title } -- by <a href="/list/${post.username}">${post.username}</h2>
...
export function show(post) {
  return layout(post.title, `
    <h1>${post.title} -- by <a href="/list/${post.username}">${post.username}</h1>
    <p>${post.body}</p>
  `)
}

export function listUserPosts(user, posts) {
  console.log('rander:list: user=', posts)
  let list = []
  for (let post of posts) {
    list.push(`
    <li>
  ...
  let content = `
  <h1>${user}'s Posts</h1>
  <p>${(posts.username==null)?'<a href="/login">Login</a> to Create a Post!':'Welcome '+posts.username+', You may <a href="/post/new">Create a Post</a> or <a href="/logout">Logout</a> !'}</p>
  <p>There are <strong>${posts.length}</strong> posts!</p>
